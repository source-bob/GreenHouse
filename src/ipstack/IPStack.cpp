...
IPStack::IPStack(const char *ssid, const char *pw) : IPStack() {
    if (cyw43_arch_init()) {
...
bool IPStack::wait_connected(uint32_t timeout_ms) {
    auto to = make_timeout_time_ms(timeout_ms);
    while (!connected && !time_reached(to)) {
