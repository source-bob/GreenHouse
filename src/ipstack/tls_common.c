...
static err_t tls_client_recv(void *arg, struct altcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    if (p->tot_len > 0) {
        if (state->response_buf && state->response_cap > 0) {
            size_t space = state->response_cap - 1 - state->response_len;
            size_t copy_n = p->tot_len < space ? p->tot_len : space;

            if (copy_n > 0) {
                pbuf_copy_partial(p, state->response_buf + state->response_len, copy_n, 0);
                state->response_len += copy_n;
                state->response_buf[state->response_len] = '\0';
            }
        }
...
bool run_tls_client_request(const uint8_t *cert, size_t cert_len,
    ...
    state->response_buf = response;
    state->response_cap = response_n;
...
bool run_tls_client_test(const uint8_t *cert, size_t cert_len,
                         const char *server, const char *request, int timeout) {
    return run_tls_client_request(cert, cert_len, server, request, timeout, NULL, 0);
}